...
MODULE_AUTHOR("Abdessamad El Abbassi <support@zevenet.com>");
...
recent_entry_init(struct recent_table *t, const union nf_inet_addr *addr,
                  u_int16_t family, u_int8_t ttl, unsigned long *first_nstamp)
{
	struct recent_entry *e;
	...
	e->ttl       = ttl;
	if(first_nstamp == NULL)
		e->stamps[0] = jiffies;
	else
		e->stamps[0] = *first_nstamp;
	e->nstamps   = 1;
	e->index     = 1;
	e->family    = family;
	if (family == NFPROTO_IPV4)
...
static void recent_entry_update(struct recent_table *t, struct recent_entry *e, unsigned long * nstamp)
{
	e->index %= ip_pkt_list_tot;
	if(nstamp == NULL)
		e->stamps[e->index++] = jiffies;
	else
        e->stamps[e->index++] = *nstamp;
...
recent_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if (e == NULL) {
		if (!(info->check_set & XT_RECENT_SET))
			goto out;
		e = recent_entry_init(t, &addr_mask, par->family, ttl,NULL);
		if (e == NULL)
			par->hotdrop = true;
		ret = !ret;
		goto out;
		     ...
		     size_t size, loff_t *loff)
{
	...
	struct recent_entry *e;
	if(size > (32+5+12*32)) //(ip+ttl+timestamp*max_timestamps
		return -EINVAL;
	char buf[size];
	...
	union nf_inet_addr addr = {};
	u_int16_t family;
	bool add, succ, addall;
	
	char *ipaddr;
        char *token;
        unsigned long nstamp = 0;
        u_int8_t ttl = 0;
        succ = false;        
        addall = false;
	...
	--size;
        if(addall)
	{
		if((ipaddr = strsep(&c,";")) != NULL){
			if (strnchr(c, size, ':') != NULL) {
			        family = NFPROTO_IPV6;
			        succ = in6_pton(ipaddr, -1, (void *)&addr,  -1,NULL);
			} else {
			        family = NFPROTO_IPV4;
			        succ   = in4_pton(ipaddr, -1, (void *)&addr, -1 ,NULL);
			}
		}
        }else
	{
	...
	if (!succ) {
		...
		if (add)
                {
                        recent_entry_init(t, &addr, family, 0, NULL);
                }else if(addall)
		 	{

                        if( (token = strsep(&c,";")) != NULL )
                        {
                                if(kstrtou8(token,10, &ttl) != 0)
                                {
				    pr_info("xt_recent: ttl not valid\n");
				    mutex_unlock(&write_mutex);
        			    spin_unlock_bh(&recent_lock);
				    return -EINVAL;
                                }
                        ...
                        if( (token = strsep(&c,";")) != NULL )
                        {
                               if(kstrtoul(token,10, &nstamp) == 0)
                                {
                                        e = recent_entry_init(t, &addr, family, ttl, &nstamp);
                                }
                        }
                        ...
                        while( (token = strsep(&c,";")) != NULL )
                        {
                                ...
                                unsigned long tmp;
                                if(kstrtoul(token,10, &tmp) == 0)
                                {
                                        recent_entry_update(t, e, &tmp);
                                }
                        }
                }
	} else {
